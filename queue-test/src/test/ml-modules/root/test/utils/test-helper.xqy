...
{
  test:load-test-file($filename, $database-id, $uri, $permissions, $collections, map:new())
};

declare function test:load-test-file($filename as xs:string, $database-id as xs:unsignedLong, $uri as xs:string, $permissions as element(sec:permission)*, $collections as xs:string*, $metadata as map:map)
{
        ...
        function() {
          xdmp:document-insert($uri, $doc, 
            <options xmlns="xdmp:document-insert">
              ...
              <collections>{ $collections ! <collection>{ . }</collection> }</collections>
